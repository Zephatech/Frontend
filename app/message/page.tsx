...
export default function MessagePage() {
    ...
    }, [authIsLoading, authIsFetching, router, userId])
...
const useGetConversations = () => {
    ...
    useEffect(() => {
        const getConversations = async () => {
            setLoading(true)
            try {
                const data = await fetch('${BACKEND_URL}/conversation', {
                    credentials: 'include',
                    method: 'GET',
                    headers: {
...
const useSendMessage = () => {
    ...
    const { messages, setMessages, selectedConversation }: any =
        useConversation((state: any) => {
            state.messages, state.setMessages, state.selectedConversation
        })
    ...
    const sendMessage = async (message: any) => {
        ...
        try {
            const res = await fetch(
                `${BACKEND_URL}/message/send/${selectedConversation?.id}`,
                {
                    method: 'POST',
                    credentials: 'include',
                    headers: {
        ...
        const getMessages = async () => {
            ...
            try {
                const data = await fetch(
                    `${BACKEND_URL}/message/${selectedConversation?.id}`,
                    {
                        credentials: 'include',
                        method: 'GET',
                        headers: {
