...
const useSendMessage = () => {
    ...
    const sendMessage = async (message: string) => {
        ...
        setLoading(true);
        try {
            const res = await fetch(`http://localhost:3001/message/send/${selectedConversation.userId}`, {
                method: "POST",
                credentials: "include",
                headers: {
                    "Content-Type": "application/json",
...
const Message = ({ message }: { message: any }) => {
    ...
    return (
        <div className={`chat ${chatClassName}`}>
            ...
            <div className={`chat-bubble text-white ${bubbleBgColor} pb-2`}>{message.content}</div>
            ...
            {!selectedConversation ? (
                ...
                <>
                    <div className='px-4 py-2 mb-2 shadow-md p-6 border-b-1 border-gray-200'>
                        <span>To: {selectedConversation.firstName} {selectedConversation.lastName}</span>
                    </div>
                    <Messages />
                    <MessageInput />
                </>
