...
function MyDialog({ isOpen, setIsOpen, profile }: { isOpen: boolean, setIsOpen: (value: boolean) => void, profile: any }) {
    return (
...
function MyDialogToSendMessage({ isOpen, setIsOpen, ownerId }: { isOpen: boolean, setIsOpen: (value: boolean) => void, ownerId: number }) {
    const [message, setMessage] = useState('Hello, I am interested in your product. Is it still avaiable?')
    const sendMessage =  async () => {
        try {
            if(ownerId === null) throw new Error('Owner Id is null')
